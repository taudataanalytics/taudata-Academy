...
import tweepy, re, itertools, os, json
...
def saveTweets(tweets, file='Tweets.json', type_="json"): #in Json Format
    if type_=='json':
...
def connect(con="twitter", key=None):
    if con.lower().strip() == "twitter":
        ...
        try:
            ...
            print(Err_)
            print("Connection failed, please check your API keys or your internet connection.")
    else:
        print('Error, connection type is not recognized (yet).')
